; =========================================================
; Cabeça e Pescoço — Carcinoma ex-Adenoma Pleomórfico
; Arquivo: scripts\CarcinomaExAdenoma.ahk
; =========================================================

Mask_CarcinomaExAdenoma() {
    prevWin := WinGetID("A")
    g := Gui("+AlwaysOnTop", "Máscara — Carcinoma ex-Adenoma Pleomórfico")
    g.SetFont("s9", "Segoe UI")
    g.MarginX := 20, g.MarginY := 20

    ; --- 1. CLASSIFICAÇÃO E GRAU ---
    g.AddGroupBox("w760 h110", "Classificação Histológica")
    g.AddText("xp+15 yp+30 w110", "Tipo de Invasão:")
    ddlInvasao := g.AddDropDownList("x140 yp-3 w240 Choose1 +Tabstop", ["não invasivo", "minimamente invasivo", "invasivo"])

    g.AddText("x400 yp+3 w110", "Grau Histológico:")
    ddlGrau := g.AddDropDownList("x520 yp-3 w240 Choose1 +Tabstop", ["baixo", "moderado", "alto"])
    ...
    g.AddText("x35 y+15 w110", "Invasão Extracaps.:")
    edtInvCaps := g.AddEdit("x140 yp-3 w60 +Tabstop"), g.AddText("x205 yp+3", "mm")
    ...
    controles := [ddlInvasao, ddlGrau, edtDim1, edtDim2, ddlMulti, edtInvCaps, ddlExt, ddlVascL, ddlVascS, ddlPeri, ddlOssea, ddlSialo, ddlTalp]
    for c in controles
    ...
    Build() {
        res := "Carcinoma ex-adenoma pleomórfico " ddlInvasao.Text " de " ddlGrau.Text " grau histológico.`n"
        res .= ". Multifocalidade: " ddlMulti.Text ".`n"
        res .= ". Dimensão da neoplasia: " (edtDim1.Value || "[]") " x " (edtDim2.Value || "[]") " cm.`n"
        res .= "- Dimensão da invasão extracapsular: " (edtInvCaps.Value || "[]") " mm.`n"
