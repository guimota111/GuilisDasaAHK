; =========================================================
; Cabeça e Pescoço — Carcinoma Adenoide Cístico
; Arquivo: scripts\AdenoideCistico.ahk
; =========================================================

Mask_AdenoideCistico() {
    prevWin := WinGetID("A")
    g := Gui("+AlwaysOnTop", "Máscara — Carcinoma Adenoide Cístico")
    g.SetFont("s9", "Segoe UI")
    g.MarginX := 20, g.MarginY := 20

    ; --- 1. CLASSIFICAÇÃO E GRADUAÇÃO ---
    g.AddGroupBox("w760 h110", "Classificação e Graduação")
    g.AddText("xp+15 yp+30 w110", "Grau Histológico:")
    ddlGrau := g.AddDropDownList("x140 yp-3 w160 Choose1 +Tabstop", ["baixo", "moderado", "alto"])

    g.AddText("x320 yp+3 w110", "Padrão Cresc.:")
    ddlPadrao := g.AddDropDownList("x410 yp-3 w160 Choose1 +Tabstop", ["cribriforme", "sólido", "tubular"])

    g.AddText("x35 y+15 w110", "Comp. Sólido:")
    edtSolido := g.AddEdit("x140 yp-3 w40 +Tabstop"), g.AddText("x185 yp+3", "%")
    g.AddText("x220 yp", "(Baixo: 0% | Interm: 1-30% | Alto: >30%)")
    ...
    controles := [ddlGrau, ddlPadrao, edtSolido, edtDim1, edtDim2, ddlMulti, ddlExt, ddlVascL, ddlVascS, ddlPeri, ddlOssea, ddlSialo, ddlTalp]
    for c in controles
    ...
    Build() {
        res := "Carcinoma adenoide cístico de grau histológico " ddlGrau.Text ".`n"
        res .= ". Padrão de crescimento: " ddlPadrao.Text ".`n"
        res .= "- Proporção do componente sólido: " (edtSolido.Value || "[]") "% do total da neoplasia.`n"
